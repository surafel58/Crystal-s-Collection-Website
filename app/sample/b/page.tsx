    ...
    <div className="space-y-24 pb-24">
      ...
      <section className="relative h-[85vh] w-full overflow-hidden">
        ...
        <div className="relative container h-full mx-auto px-4 md:px-6 flex items-center justify-center text-center">
           <FadeIn className="max-w-3xl space-y-8 bg-white/80 backdrop-blur-sm p-12 rounded-sm shadow-xl" delay={0.2}>
             ...
             <Button asChild size="lg" className="rounded-full bg-amber-800 hover:bg-amber-900 text-white px-10 py-6 text-lg transition-all hover:scale-105 active:scale-95">
      ...
      <section className="container px-4 md:px-6 mx-auto">
        <FadeIn className="text-center mb-16 space-y-4" delay={0.2}>
        ...
        <StaggerContainer className="grid grid-cols-1 md:grid-cols-3 gap-10">
          {featuredProducts.map((product) => (
            <StaggerItem key={product.id}>
              <Link href={`/sample/b/product/${product.id}`} className="group block">
                <div className="relative aspect-[4/5] bg-stone-100 rounded-t-2xl overflow-hidden mb-6 shadow-sm transition-shadow group-hover:shadow-md">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-4 bg-white/95 backdrop-blur translate-y-full group-hover:translate-y-0 transition-transform duration-300 flex gap-2">
                     <Button 
                       variant="outline" 
                       className="flex-1 border-stone-800 text-stone-800 hover:bg-stone-100 rounded-full text-xs h-9 transition-transform hover:scale-105 active:scale-95"
                     >
                       View Details
                     </Button>
                     <Button 
                       className="flex-1 bg-amber-800 hover:bg-amber-900 text-white rounded-full text-xs h-9 transition-transform hover:scale-105 active:scale-95"
      ...
      <section className="bg-[#f0efe9] py-24">
        <div className="container px-4 md:px-6 mx-auto grid md:grid-cols-2 gap-16 items-center">
          <FadeIn className="space-y-8" delay={0.2}>
             ...
             <Button asChild variant="outline" className="border-amber-800 text-amber-800 hover:bg-amber-800 hover:text-white rounded-full px-8 transition-transform hover:scale-105 active:scale-95">
               <Link href="/sample/b/about">Our Philosophy</Link>
             </Button>
          </FadeIn>
          <FadeIn className="relative aspect-square rounded-full overflow-hidden border-8 border-white shadow-xl" delay={0.4}>
             <Image 
